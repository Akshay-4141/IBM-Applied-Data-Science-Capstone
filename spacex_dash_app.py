...
def update_pie_chart(entered_site):
    if entered_site == 'ALL':
        # When "All Sites" are selected, show the count of successful launches for each site
        all_sites_success_count = spacex_df[spacex_df['class'] == 1].groupby('Launch Site').size().reset_index(name='Count')
        fig = px.pie(all_sites_success_count, names='Launch Site', values='Count',
                     title='Successful Launches Count for All Sites')
    else:
        # When a specific site is selected, show the Success vs. Failed counts for that site
